 ...
 * TeacherEstimatedBreakdownMaybe components. You can then make customization
...
const estimatedTransaction = (bookingStart, bookingEnd, lineItems, userRole, sessionHour) => {
    const now = new Date();
    ...
    let tempDisplayEnd = new Date(bookingEnd);
    tempDisplayEnd.setDate(tempDisplayEnd.getDate() - 1);
...
const TeacherEstimatedBreakdownMaybe = props => {
    const { unitType, startDate, endDate, sessionHour, startTime, endTime } = props.bookingData;
    ...
    const tx =
        startDate && endDate && lineItems
            ? estimatedTransaction(startDate, endDate, lineItems, userRole, sessionHour)
            : null;
...
export default TeacherEstimatedBreakdownMaybe;
